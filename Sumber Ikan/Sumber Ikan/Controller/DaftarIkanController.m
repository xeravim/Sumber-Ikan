...
#import "DaftarIkanController.h"
#import "Item.h"
#import "ItemCategory.h"

@interface DaftarIkanController (){
...
@implementation DaftarIkanController
...
- (void)reload:(__unused id)sender {
    ...
    task = [Item listAliveItemsWithBlock:^(NSArray *items, NSError *error) {
        if (!error) {
            [self populateData];
        }
    }];
...
- (void)reloadItems{
    ...
    NSURLSessionTask *task = [Item listAliveItemsWithBlock:^(NSArray *items, NSError *error) {
        if (!error) {
            [self populateData];
        }
    }];
...
//    }
//    
//    NSArray *itemsFromResponse = [JsonManager getSavedJsonData:@"aliveItems"];
...
- (void)initAppearance{
    ...
    NSArray* items=[JsonManager getSavedJsonData:@"aliveItems"];
